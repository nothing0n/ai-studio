...
一个可以部署在 GitHub Pages 的个人 AI 聊天工作台：管家分配问题，关卡策划、系统策划、数值策划接手；也可以创建自己的角色。电脑和手机共用一个网页，聊天通过 GitHub API 同步到你自己的私有仓库。
...
2. 回到工作台开始聊天。管家先根据明确的角色指令和关键词分配问题；首次问题不明确时，可调用默认模型选择角色。短追问保持当前角色。点击左侧角色，可以手动切换并沿用当前会话。
